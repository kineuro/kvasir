...
// runtime in kvasir.json; the loopback name of a Kvasir in a container; and a
// started model that admission refuses, shown and no purpose's default.
...
import { presetsText } from "../src/runtime.js";
...
async function kvasir(
  ...
  opts: { gate?: boolean; hostAlias?: string; ready?: boolean; purposes?: unknown[] } = {},
) {
  ...
  const k: Kvasir = build(
    ...
    { local: { statfs: ROOMY, pollMs: 50 }, runtime: { pollMs: 40, askAgainMs: 400, settleMs: 0 } },
  );
...
/** A model downloaded as Kvasir lays one out: its files written, a size of null for one listed but not there, its row done. */
function downloaded(k: Kvasir, name: string, files: Record<string, number | null>): number {
  const commit = createHash("sha1").update(name).digest("hex");
  const path = join(k.local.location(), `acme--${name}`, commit);
  const list = Object.entries(files).map(([file, size]) => {
    if (size !== null) {
      mkdirSync(dirname(join(path, file)), { recursive: true });
      writeFileSync(join(path, file), Buffer.alloc(size));
    }
    return { path: file, size: size ?? 1, sha256: null };
  });
