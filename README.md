...
The lifecycle of a candidate model (Wave 5 §9.5) is four states in Kvasir's own database: registered, admitted, promoted, retired. A candidate is registered from a fine-tune job with its recipe, or by hand; the suite admits it through the same runner as `POST /v1/admission/run`, and a failed suite records the failures and leaves it registered; a promotion needs an admission record that passed and is refused with a sentence without one; on promotion the candidate becomes the model the purposes route to on its backend and the previously promoted one is retired; a retired candidate routes nothing. Every transition is a row with who did it and when. `GET /v1/models/lifecycle` (any role; `?events=1` adds the transitions) lists every candidate with its records and the promoted model per backend, which the desk's Teaching page reads; `POST /v1/models/lifecycle {model, backend, source?, notes?}`, `POST /v1/models/lifecycle/{id}/admit`, `POST /v1/models/lifecycle/{id}/promote {proposal?: {id, principal}}` and `POST /v1/models/lifecycle/{id}/retire` are an admin's. The same from the command line: `kvasir models lifecycle list | register --model ID --backend ID [--job N --recipe JSON] | admit --id N | promote --id N [--proposal ID] | retire --id N`.
